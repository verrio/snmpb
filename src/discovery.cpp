    ...
    for (int t = 0; t < 2; t++) // for all transports
    {
        ...
        else
        {
            ...
            Address::version_type v = 
                UdpAddress(address_str.toUtf8().data()).get_ip_version(); 
            if (!(((t == 0) && (v == Address::version_ipv4) && (v4 == true)) ||
                  ((t == 1) && (v == Address::version_ipv6) && (v6 == true))))
                continue;
        }

        UdpAddress start_address(address_str.toUtf8().data());
...
void Discovery::AddAgentToProfiles(void)
{
    ...
    {
        strcpy(buf, item_list[i]->text(1).toUtf8().data());
        QString address(strtok(buf, "/"));

        s->APManagerObj()->Add(item_list[i]->text(0).isEmpty()?
                               address:item_list[i]->text(0), 
                               address, 
                               QString(strstr(item_list[i]->text(1).toUtf8().data(), "/") + 1), 
                               strstr(item_list[i]->text(2).toUtf8().data(), 
...
void Discovery::Discover(void)
{
    ...
    {
        IpAddress addr_from(s->MainUI()->DiscoveryFrom->text().toUtf8().data());
        IpAddress addr_to(s->MainUI()->DiscoveryTo->text().toUtf8().data());
