...
void Agent::Init(void)
{
    ...
    status = saveBootCounter(s->GetBootCounterConfigFile().toUtf8().data(), 
                             engineId, snmpEngineBoots);
    if (status != SNMPv3_OK)
    {
    ...
    usm->load_users(s->GetUsmUsersConfigFile().toUtf8().data());
}
...
void Agent::SelectAgentProfile(QString *prefprofile, int prefproto)
{
    ...
    if (ap)
    {
        ...
        if (prefprofile)
        {
            ...
            // update the combobox
            int index = s->MainUI()->AgentProfile->findText(prefprofile->toUtf8());
...
{    
    ...
    UdpAddress address(address_str.toUtf8().data());
                                     ...
                                     AgentProfile *ap)
{
    ...
    else if ((v == version1) || (v == version2c))
    {
        ...
        // set the community
        ctarget->set_readcommunity( ap->GetReadComm().toUtf8().data());
        ctarget->set_writecommunity( ap->GetWriteComm().toUtf8().data());
    }
                                 ...
                                 Pdu *p, AgentProfile *ap, bool usevblist)
{
    ...
    if (v == version3)
    {
        ...
        p->set_context_name(ap->GetContextName().toUtf8().data());
        p->set_context_engine_id(ap->GetContextEngineID().toUtf8().data());
    }
