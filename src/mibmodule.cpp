...
void MibModule::RebuildTotalList(int restart)
{
    ...
    { 
        smiInit(NULL);
        smiReadConfig(s->GetPathConfigFile().toUtf8().data(), NULL);
    }
         ...
         dir = mystrtok_r(NULL, sep, &svptr))
    {
        ...
        while ( it.hasNext() ) {
            ...
            QString ext = QFileInfo(fi->toUtf8()).suffix();
            if (!(((str = strstr(fi->toUtf8().data(), "-orig")) != NULL)
                && (strlen(str) == 5)) &&
                ((ext.isEmpty() && 
                ((((str = strstr(fi->toUtf8().data(), "-MIB")) != NULL)
                && (strlen(str) == 4)) || 
                (((str = strstr(fi->toUtf8().data(), "-SMI")) != NULL)
                && (strlen(str) == 4)) || 
                (((str = strstr(fi->toUtf8().data(), "-TC")) != NULL)
                && (strlen(str) == 3)) || 
                (((str = strstr(fi->toUtf8().data(), "-TYPES")) != NULL)
                && (strlen(str) == 6)) || 
                (((str = strstr(fi->toUtf8().data(), "-PIB")) != NULL)
                && (strlen(str) == 4)))) || 
                ...
                (ext == "SMI") || (ext == "MIB") || (ext == "PIB")))
            {
                ...
                QString modPath = QString(dir) + QDir::separator() + *fi;
                char *mod = smiLoadModule(modPath.toUtf8().data());
                ...
                module += QFileInfo(fi->toUtf8()).baseName();
...
QString MibModule::LoadBestModule(QString oid)
{
    ...
    if (best_file != "")
    {
        ...
        // Load the module
        Wanted.append(best_file.toUtf8().data());
        Refresh();
        SaveWantedModules();
        s->TabSelected();
    }
...
void MibModule::RebuildLoadedList(void)
{
    ...
    while (mod)
    {
        ...
        QStringList values;
        values << lmodule->name.toUtf8().data() << required
...
void MibModule::AddModule(void)
{
    ...
    for (int i = 0; i < item_list.size(); i++)
        Wanted.append(item_list[i]->text(0).toUtf8().data());
...
void MibModule::SaveWantedModules(void)
{
    ...
    for (int i = 0; i < Wanted.size(); i++)
        out << "load " << QFileInfo(Wanted[i]).baseName() << endl;
    mibs.close();
}
...
void MibModule::RefreshPathChange(void)
{
    ...
    smiReadConfig(s->GetMibConfigFile().toUtf8().data(), NULL);

    Wanted.clear();
    for(SmiModule *mod = smiGetFirstModule(); 
        mod; mod = smiGetNextModule(mod)) {
        Wanted.append(QFileInfo(mod->path).baseName());
    }
