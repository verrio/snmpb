...
void USMProfileManager::Execute (void)
{
    if(upw.exec() == QDialog::Accepted)
    {
        ...
        {
            usm->add_usm_user(OctetStr(users[i]->GetName().toUtf8().data()),
                              UiAuthToLibAuth(users[i]->GetAuthProto()),
                              UiPrivToLibPriv(users[i]->GetPrivProto()),
                              OctetStr((const unsigned char *)users[i]->GetAuthPass().toUtf8().data(), 
                                       users[i]->GetAuthPass().toUtf8().length()), 
                              OctetStr((const unsigned char *)users[i]->GetPrivPass().toUtf8().data(), 
                                       users[i]->GetPrivPass().toUtf8().length()));
        }

        // ... then save it to file.    
        usm->save_users(s->GetUsmUsersConfigFile().toUtf8().data());
    }
}
